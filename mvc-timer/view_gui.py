...
class GuiTimerView(TimerView):
    ...
    def __init__(self, model):
        ...
        self.minutes_up_button = ttk.Button(
            self.root, text="▲", command=self.increment_minutes, state="normal"
        )
        self.minutes_up_button.grid(row=0, column=0, padx=5, pady=5)

        self.minutes_down_button = ttk.Button(
            self.root, text="▼", command=self.decrement_minutes, state="normal"
        )
        self.minutes_down_button.grid(row=1, column=0, padx=5, pady=5)

        self.seconds_up_button = ttk.Button(
            self.root, text="▲", command=self.increment_seconds, state="normal"
        )
        self.seconds_up_button.grid(row=0, column=2, padx=5, pady=5)

        self.seconds_down_button = ttk.Button(
            self.root, text="▼", command=self.decrement_seconds, state="normal"
        )
        ...
        self.stop_button = ttk.Button(
            self.root, text="⏹", command=self.stop, state="disabled"
        )
        self.stop_button.grid(row=2, column=1, padx=5, pady=5)

        self.pause_button = ttk.Button(
            self.root, text="⏸", command=self.pause, state="disabled"
        )
        self.pause_button.grid(row=2, column=2, padx=5, pady=5)

        self.sound_button = ttk.Button(
            self.root, text="Select Sound", command=self.select_sound
        )
        self.sound_button.grid(row=3, column=0, columnspan=3, padx=5, pady=5)
    ...
    #def disable_enable_buttons(self, *buttons):
    #    """Disable or enable the given buttons."""
    ...
    def start(self):
        ...
        #    self.stop_button,
        #    self.pause_button,
        #    self.minutes_up_button,
        #    self.minutes_down_button,
        #    self.seconds_up_button,
        #    self.seconds_down_button,
        #)
